...
export class Code {
    public static async generate(uid: string, expiresIn: string): Promise<{ code: number; token: string }> {
        const code = Math.floor(Math.random() * (9999 - 1000) + 1000);

        const { payload, token, hash } = await Token.generate(uid, expiresIn, "Verification-Token");
    ...
    public static async verify(code: number, token: string): Promise<string> {
        try {
            ...
            if (code !== verification.code) {
                throw ServerError.AuthenticationError([
                    { cause: "Incorrect Code", message: "Incorrect code entered. Please try again" }
                ]);
            }
            ...
            if (e instanceof TokenExpiredError) {
                e = ServerError.AuthenticationError([{ cause: "Expired Code", message: "Code provided is expired" }]);
            }
...
export class Token {
    public static async generate(
        ...
        expiresIn: string,
        type: string
    ): Promise<{ payload: CustomJwtPayload; token: string; hash: string }> {
